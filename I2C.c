...
{                                                  /* SCL Calculation using the user-defined Prescaler and Bit Rate
    ...
    TWBR = (config_ptr->bit_rate);                 /* Bit-rate selected*/
	TWSR = (config_ptr->prescaler << TWPS0);       /* prescaler value selected */


    TWAR = ((config_ptr->address & 0x7F) << 1);    /* Two Wire Bus address my address if any master device want to call me:
